...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func makeWebView(configuration: WKWebViewConfiguration?) -> WKWebView {
        ...
        if let configuration {
            ...
            let userScript = WKUserScript(source: Self.inputHelperScript,
    ...
    private func navigate(to raw: String) {
        ...
        guard !trimmed.isEmpty, let webView = tabManager.activeWebView else { return }
        ...
        if target == nil {
            ...
            target = URL(string: "https://www.startpage.com/sp/search?query=\(query)")
        }
        if let target {
